        ...
        foreach ($semester as $s => $groups) {
            if (count($groups) > 0) {
                foreach ($groups as $g) {
                    ...
                    $result = $db->query("SELECT MAX(`vlv_entry`.`last_change`) AS `last_change` FROM `vlv_entry` INNER JOIN `vlv_entry2stud` ON `vlv_entry`.`id` = `vlv_entry2stud`.`vlv_id` WHERE `vlv_entry2stud`.`studiengang` = '".$sgang."' AND `vlv_entry2stud`.`seminargruppe` = '".$g."' AND `vlv_entry2stud`.`semester` = '".$g."' AND `vlv_entry`.`last_change` <= DATE_FORMAT(NOW(),'%Y-%m-%d');");
                    $row = $result->fetch_assoc();
                    print "\n\t<url>\n\t\t<lastmod>{$row['last_change']}</lastmod>\n\t\t<loc>https://vlv-ilmenau.de/#!" . $sgang . "|" . $s . "|" . $g . "</loc>\n\t\t<changefreq>daily</changefreq>\n\t\t<priority>0.2</priority>\n\t</url>";
                }
            } else {
                // Last-Change herausfinden
                $result = $db->query("SELECT MAX(`vlv_entry`.`last_change`) AS `last_change` FROM `vlv_entry` INNER JOIN `vlv_entry2stud` ON `vlv_entry`.`id` = `vlv_entry2stud`.`vlv_id` WHERE `vlv_entry2stud`.`studiengang` = '".$sgang."' AND `vlv_entry2stud`.`semester` = '".$g."' AND `vlv_entry`.`last_change` <= DATE_FORMAT(NOW(),'%Y-%m-%d');");
                $row = $result->fetch_assoc();
