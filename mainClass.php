...
class mainClass {
	...
	public function getHeader() {
		...
		$url = urldecode(str_replace("?_escaped_fragment_=","#!",$url));
	?><!DOCTYPE html>
<html lang='de'>
	<head>
		<meta charset="UTF-8" />
...
<?php
if(
	($_SERVER['SCRIPT_NAME'] == "/settings.php") OR
	($_SERVER['SCRIPT_NAME'] == "/login.php") OR
	($_SERVER['SCRIPT_NAME'] == "/register.php") OR
	($_SERVER['SCRIPT_NAME'] == "/literatur.php")
)
	...
	<body>
		<div class="container" id="page">
			<div id="header">
				...
				<div id="mainmenu">
					<ul id="yw0">
						<li<?php if($_SERVER['SCRIPT_NAME'] == "/index.php") echo " class='active'"; ?>><a href="/">Vorlesungsverzeichnis</a></li>
<?php if($this->getUser()) { ?>
						<li<?php if($_SERVER['SCRIPT_NAME'] == "/settings.php") echo " class='active'"; ?>><a href="settings.php">Einstellungen</a></li>
<?php if($this->isSu()) { ?>			<li<?php if($_SERVER['SCRIPT_NAME'] == "/literatur.php") echo " class='active'"; ?>><a href="literatur.php">Literatur</a></li> <?php } ?>						
						<li><a href="logout.php">Logout (<?php echo $this->getUser();?>)</a></li>
<?php } else { ?>						
						<li<?php if($_SERVER['SCRIPT_NAME'] == "/login.php") echo " class='active'"; ?>><a href="login.php">Login</a></li>
						<li<?php if(($_SERVER['SCRIPT_NAME'] == "/register.php") OR ($_SERVER['SCRIPT_NAME'] == "/register_done.php")) echo " class='active'"; ?>><a href="register.php">Registrierung</a></li>
<?php } ?>					
						<li<?php if($_SERVER['SCRIPT_NAME'] == "/impressum.php") echo " class='active'"; ?>><a href="impressum.php">Impressum</a></li>
					</ul>
				</div><!-- mainmenu -->
			<div id="content"><?php
	}
	...
	public function login() {
		...
		elseif(isset($_POST['user']) AND isset($_POST['password'])) {
			$result = $this->db->query("SELECT `uid`,`password` FROM `user` WHERE `uname`='".$this->db->real_escape_string($_POST['user'])."' AND `block`=0");
	...
	public function register() {
		...
		elseif(isset($_POST['user']) AND isset($_POST['password']) AND isset($_POST['password_verify']) AND isset($_POST['email'])) {
			$result = $this->db->query("SELECT * FROM `user` WHERE `uname`='".$this->db->real_escape_string($_POST['user'])."'");
			if($result->num_rows == 1)
				$this->registerErrorUser = "Nutzername ist bereits vorhanden";
			
			if(!preg_match('/^[a-zA-Z0-9-_]{4,50}$/',$_POST['user']))
			...
			$result = $this->db->query("SELECT * FROM `user` WHERE `email`='".$this->db->real_escape_string(strtolower(trim($_POST['email'])))."'");
			if($result->num_rows == 1)
				$this->registerErrorEmail = "Emailadresse ist bereits vorhanden";

			if(!preg_match('/^[a-z0-9._%+-]+@([a-z0-9]+\.)?tu-ilmenau.de$/',strtolower($_POST['email'])))
			...
			if(!$this->registerErrorUser AND !$this->registerErrorEmail AND !$this->registerErrorPassword) {
				...
				$this->sendMail($_POST['email'],'Verifizieren Sie Ihren Account auf vlv-ilmenau.de',$message);
				$this->db->real_query("INSERT INTO `user` (`uname`, `password`, `email`, `verify`, `iCal_string`, `block`) VALUES ('".$this->db->real_escape_string(trim($_POST['user']))."', '".$this->db->real_escape_string(password_hash($_POST['password'],PASSWORD_DEFAULT))."','".$this->db->real_escape_string(strtolower(trim($_POST['email'])))."','".$this->db->real_escape_string($verify)."','".$this->db->real_escape_string($ical)."',1)");
	...
	public function verify() {
		if(isset($_SESSION['uid']))
			$this->goHeader();
		if(isset($_GET['c'])) {
			$result = $this->db->query("SELECT `uid` FROM `user` WHERE `verify` = '".$this->db->real_escape_string($_GET['c'])."' AND `block` = 1");
			if($result->num_rows == 1) {
				$row = $result->fetch_assoc();
	...
	public function sendMail ($to, $subject, $message) {
		...
		$gpg = new gnupg();
		$gpg->addsignkey("C74351043DD0EE1B543744998FD75D3B61296EE4","oobae9rie6oP2coo");
