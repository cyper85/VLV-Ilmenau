	...
	public function getEvent() {
			...
			if(is_array($v)) {
				foreach($v as $subV)
					$return .= mb_convert_encoding(
							chunk_split(mb_convert_encoding($k.":".str_replace(",","\,",str_replace(";","\;",preg_replace("/[\t]/","\\t",preg_replace("/[\n\r]+/","\\r\\n",str_replace("\\","\\\\",$subV))))),"ISO-8859-15","UTF-8"),60,"\r\n ")."\r\n",
						"UTF-8","ISO-8859-15");
			}
			else
				$return .= mb_convert_encoding(
		...
		foreach($this->unescape as $k => $v) {
			if(is_array($v)) {
				foreach($v as $subV)
					$return .= mb_convert_encoding(
							chunk_split(mb_convert_encoding($k.":".$subV,"ISO-8859-15","UTF-8"),60,"\r\n ")."\r\n",
						"UTF-8","ISO-8859-15");
			}
			else
			$return .= mb_convert_encoding(
					chunk_split(mb_convert_encoding($k.":".$v,"ISO-8859-15","UTF-8"),60,"\r\n ")."\r\n",
				   "UTF-8","ISO-8859-15");
		}
		return	"BEGIN:VEVENT\r\n".
			$return.
...
$row = $result->fetch_assoc();
if(isset($_GET['id'])) {
	$command = $db->query("SELECT UNIX_TIMESTAMP(`lastupdate`) as `lastupdate` FROM `user` WHERE `iCal` = 0 AND `iCal_string` = '".$db->real_escape_string($_GET['id'])."'");
	if($command->num_rows == 1) {
...
if(isset($_SERVER['HTTP_IF_NONE_MATCH']) AND (trim($_SERVER['HTTP_IF_NONE_MATCH']) == md5(gmdate("D, d M Y H:i:s", $row['value'])." GMT"))) {
       header("HTTP/1.1 304 Not Modified");
       exit;
}
elseif(isset($_SERVER['HTTP_IF_MODIFIED_SINCE']) AND (trim($_SERVER['HTTP_IF_MODIFIED_SINCE']) == $row['value'])) {
       header("HTTP/1.1 304 Not Modified");
       exit;
}
...
if(isset($_GET['Studiengang']) AND isset($_GET['Semester'])) {
...
elseif(isset($_GET['id'])) {
	$command = $db->query("SELECT `uid`,`uname` FROM `user` WHERE `iCal` = 0 AND `iCal_string` = '".$db->real_escape_string($_GET['id'])."'");
	if($command->num_rows == 1) {
		$row = $command->fetch_assoc();
